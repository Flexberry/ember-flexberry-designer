...
export default Ember.Service.extend({
    ...
    let promise = new Ember.RSVP.Promise(function(resolve, reject) {
      store.query(modelName, builder.build()).then((result) => {
        if (!result || result.get('length') !== undefined && result.get('length') === 0) {
          let system = store.createRecord(modelName, {
            name: systemName,
            stage: stage
  ...
  */
  getAutogeneratedSystem() {
    let stage = this.get('context.system');

    Ember.assert('Autogenerated system for stage is not set.', stage);

export default {
  projections: {
    ApplicationClassView: {
      name: {
        __caption__: ''
      },
      nameStr: {
        __caption__: ''
      },
      stereotype: {
        __caption__: ''
      },
      containersStr: {
        __caption__: ''
      }
    },
    AttributesChangeView: {
      attributesStr: {
        __caption__: ''
      },
      name: {
        __caption__: ''
      },
      nameStr: {
        __caption__: ''
      },
      stereotype: {
        __caption__: ''
      },
      attributes: {
        __caption__: '',
        name: {
          __caption__: ''
        },
        type: {
          __caption__: ''
        },
        caption: {
          __caption__: ''
        },
        description: {
          __caption__: ''
        },
        class: {
          __caption__: ''
        }
      },
      views: {
        __caption__: '',
        name: {
          __caption__: ''
        },
        properties: {
          __caption__: ''
        },
        definition: {
          __caption__: ''
        },
        class: {
          __caption__: ''
        }
      }
    },
    AttributesView: {
      attributesStr: {
        __caption__: ''
      },
      name: {
        __caption__: ''
      },
      nameStr: {
        __caption__: ''
      },
      stereotype: {
        __caption__: ''
      },
      attributes: {
        __caption__: '',
        name: {
          __caption__: ''
        },
        type: {
          __caption__: ''
        },
        class: {
          __caption__: ''
        }
      }
    },
    AuditPrototyping: {
      stereotype: {
        __caption__: ''
      },
      nameStr: {
        __caption__: ''
      },
      name: {
        __caption__: ''
      },
      auditEnabled: {
        __caption__: ''
      },
      addAuditFields: {
        __caption__: ''
      },
      useDefaultView: {
        __caption__: ''
      },
      insertAudit: {
        __caption__: ''
      },
      insertAuditViewName: {
        __caption__: ''
      },
      deleteAudit: {
        __caption__: ''
      },
      deleteAuditViewName: {
        __caption__: ''
      },
      updateAudit: {
        __caption__: ''
      },
      updateAuditViewName: {
        __caption__: ''
      },
      selectAudit: {
        __caption__: ''
      },
      selectAuditViewName: {
        __caption__: ''
      },
      attributesStr: {
        __caption__: ''
      },
      stored: {
        __caption__: ''
      },
      attributes: {
        __caption__: '',
        accessModifier: {
          __caption__: ''
        },
        stored: {
          __caption__: ''
        },
        name: {
          __caption__: ''
        },
        description: {
          __caption__: ''
        },
        type: {
          __caption__: ''
        },
        defaultValue: {
          __caption__: ''
        },
        notNull: {
          __caption__: ''
        },
        dataServiceExpressionXML: {
          __caption__: 'DataService Expression'
        },
        order: {
          __caption__: ''
        },
        storage: {
          __caption__: ''
        },
        trim: {
          __caption__: ''
        }
      },
      views: {
        __caption__: '',
        name: {
          __caption__: ''
        },
        definition: {
          __caption__: ''
        }
      }
    },
    BusinessServer: {
      nameStr: {
        __caption__: ''
      },
      stereotype: {
        __caption__: ''
      },
      attributes: {
        __caption__: '',
        accessModifier: {
          __caption__: ''
        },
        name: {
          __caption__: ''
        },
        description: {
          __caption__: ''
        },
        type: {
          __caption__: ''
        },
        defaultValue: {
          __caption__: ''
        },
        pBCustomAttributes: {
          __caption__: ''
        },
        pBGetEnd: {
          __caption__: ''
        },
        pBGetStart: {
          __caption__: ''
        },
        pBSetEnd: {
          __caption__: ''
        },
        pBSetStart: {
          __caption__: ''
        }
      },
      methods: {
        __caption__: '',
        accessModifier: {
          __caption__: ''
        },
        name: {
          __caption__: ''
        },
        description: {
          __caption__: ''
        },
        type: {
          __caption__: ''
        },
        parametersStr: {
          __caption__: ''
        },
        isEvent: {
          __caption__: ''
        },
        accessType: {
          __caption__: ''
        },
        pBCustomAttributes: {
          __caption__: ''
        },
        parameters: {
          __caption__: '',
          name: {
          __caption__: ''
        },
          modifier: {
          __caption__: ''
        },
          type: {
          __caption__: ''
        },
          description: {
          __caption__: ''
        }
        }
      }
    },
    BusinessServerExtra: {
      nameStr: {
        __caption__: ''
      },
      name: {
        __caption__: ''
      },
      stereotype: {
        __caption__: ''
      },
      views: {
        __caption__: '',
        name: {
          __caption__: ''
        }
      }
    },
    ClassV: {
      referenceCount: {
        __caption__: 'Количество упоминаний'
      },
      stereotype: {
        __caption__: 'Стереотип'
      },
      nameStr: {
        __caption__: 'Название'
      },
      description: {
        __caption__: 'Описание'
      },
      packet: {
        __caption__: ''
      },
      namespacePostfix: {
        __caption__: 'Namespace postfix'
      },
      stage: {
        __caption__: '',
        name: {
          __caption__: ''
        }
      }
    },
    EditApplication: {
      name: {
        __caption__: ''
      },
      description: {
        __caption__: ''
      },
      caption: {
        __caption__: ''
      },
      standartDesktop: {
        __caption__: ''
      },
      namespacePostfix: {
        __caption__: ''
      },
      pBCustomAttributes: {
        __caption__: ''
      },
      disableAllRightChecks: {
        __caption__: ''
      },
      containers: {
        __caption__: ''
      },
      appConfigFile: {
        __caption__: ''
      },
      auditConnectionStringName: {
        __caption__: 'Имя строки соединения с БД аудита'
      },
      auditWinServiceUrl: {
        __caption__: 'Адрес сервиса аудита'
      },
      writeSessions: {
        __caption__: 'Вести аудит сессий пользователей'
      },
      attributes: {
        __caption__: '',
        accessModifier: {
          __caption__: ''
        },
        name: {
          __caption__: ''
        },
        description: {
          __caption__: ''
        },
        type: {
          __caption__: ''
        },
        defaultValue: {
          __caption__: ''
        },
        pBCustomAttributes: {
          __caption__: ''
        },
        pBGetEnd: {
          __caption__: ''
        },
        pBGetStart: {
          __caption__: ''
        },
        pBSetEnd: {
          __caption__: ''
        },
        pBSetStart: {
          __caption__: ''
        }
      },
      methods: {
        __caption__: '',
        accessModifier: {
          __caption__: ''
        },
        name: {
          __caption__: ''
        },
        description: {
          __caption__: ''
        },
        type: {
          __caption__: ''
        },
        parametersStr: {
          __caption__: ''
        },
        pBCustomAttributes: {
          __caption__: ''
        },
        isEvent: {
          __caption__: ''
        },
        accessType: {
          __caption__: ''
        },
        parameters: {
          __caption__: '',
          name: {
          __caption__: ''
        },
          modifier: {
          __caption__: ''
        },
          type: {
          __caption__: ''
        },
          description: {
          __caption__: ''
        }
        }
      }
    },
    EditBusinessServer: {
      name: {
        __caption__: ''
      },
      description: {
        __caption__: ''
      },
      packet: {
        __caption__: ''
      },
      namespacePostfix: {
        __caption__: ''
      },
      pBMembers: {
        __caption__: ''
      },
      generateComPlusServer: {
        __caption__: ''
      },
      comPlusServerOptions: {
        __caption__: ''
      },
      generateHttpRemoteServer: {
        __caption__: ''
      },
      publishToEBSD: {
        __caption__: ''
      }
    },
    EditDataObjectClass: {
      accessType: {
        __caption__: 'Access check type'
      },
      addAuditFields: {
        __caption__: 'Добавить поля аудита'
      },
      auditEnabled: {
        __caption__: 'Вести аудит класса'
      },
      autoAltered: {
        __caption__: ''
      },
      bSClass: {
        __caption__: 'Business Server'
      },
      bSEvents: {
        __caption__: 'BS Events'
      },
      businessServerClass: {
        __caption__: '',
        stereotype: {
          __caption__: ''
        }
      },
      caption: {
        __caption__: ''
      },
      deleteAudit: {
        __caption__: 'Вести аудит операции удаления'
      },
      deleteAuditViewName: {
        __caption__: 'Имя представления для аудита операции удаления'
      },
      description: {
        __caption__: ''
      },
      formUrl: {
        __caption__: 'Путь к форме просмотра объекта'
      },
      insertAudit: {
        __caption__: 'Вести аудит операции создания'
      },
      insertAuditViewName: {
        __caption__: 'Имя представления для аудита операции создания'
      },
      loadingOrderXML: {
        __caption__: 'LoadingOrder'
      },
      name: {
        __caption__: ''
      },
      namespacePostfix: {
        __caption__: ''
      },
      packet: {
        __caption__: ''
      },
      pBCustomAttributes: {
        __caption__: ''
      },
      pBMembers: {
        __caption__: ''
      },
      primaryKeyStorage: {
        __caption__: ''
      },
      publishToEBSD: {
        __caption__: ''
      },
      selectAudit: {
        __caption__: 'Вести аудит операции чтения'
      },
      selectAuditViewName: {
        __caption__: 'Имя представления для аудита операции чтения'
      },
      storage: {
        __caption__: ''
      },
      publishName: {
        __caption__: ''
      },
      stored: {
        __caption__: ''
      },
      trim: {
        __caption__: ''
      },
      updateAudit: {
        __caption__: 'Вести аудит операции изменения'
      },
      updateAuditViewName: {
        __caption__: 'Имя представления для аудита операции изменения'
      },
      useDefaultView: {
        __caption__: 'Использовать представление по умолчанию'
      },
      writeMode: {
        __caption__: 'Режим записи аудита'
      },
      attributes: {
        __caption__: '',
        accessModifier: {
          __caption__: ''
        },
        stored: {
          __caption__: ''
        },
        name: {
          __caption__: ''
        },
        description: {
          __caption__: ''
        },
        type: {
          __caption__: ''
        },
        defaultValue: {
          __caption__: ''
        },
        notNull: {
          __caption__: ''
        },
        dataServiceExpressionXML: {
          __caption__: 'DataService Expression'
        },
        storage: {
          __caption__: ''
        },
        publishName: {
          __caption__: ''
        },
        hint: {
          __caption__: ''
        },
        order: {
          __caption__: ''
        },
        trim: {
          __caption__: ''
        },
        pBCustomAttributes: {
          __caption__: ''
        },
        pBGetEnd: {
          __caption__: ''
        },
        pBGetStart: {
          __caption__: ''
        },
        pBSetEnd: {
          __caption__: ''
        },
        pBSetStart: {
          __caption__: ''
        },
        autoincrement: {
          __caption__: ''
        }
      },
      methods: {
        __caption__: '',
        accessModifier: {
          __caption__: ''
        },
        type: {
          __caption__: ''
        },
        parametersStr: {
          __caption__: ''
        },
        isEvent: {
          __caption__: ''
        },
        pBCustomAttributes: {
          __caption__: ''
        },
        name: {
          __caption__: ''
        },
        description: {
          __caption__: ''
        },
        parameters: {
          __caption__: '',
          name: {
          __caption__: ''
        },
          modifier: {
          __caption__: ''
        },
          type: {
          __caption__: ''
        },
          description: {
          __caption__: ''
        }
        }
      },
      views: {
        __caption__: '',
        name: {
          __caption__: ''
        },
        description: {
          __caption__: ''
        },
        properties: {
          __caption__: ''
        }
      }
    },
    EditEditForm: {
      name: {
        __caption__: ''
      },
      description: {
        __caption__: ''
      },
      caption: {
        __caption__: ''
      },
      generateDependedForm: {
        __caption__: ''
      },
      fixDependedForm: {
        __caption__: ''
      },
      packet: {
        __caption__: ''
      },
      namespacePostfix: {
        __caption__: ''
      },
      pBCustomAttributes: {
        __caption__: ''
      },
      pBMembers: {
        __caption__: ''
      },
      propertyLookup: {
        __caption__: ''
      },
      editFormOperations: {
        __caption__: ''
      },
      printContainer: {
        __caption__: ''
      },
      scriptName: {
        __caption__: ''
      },
      publishToEBSD: {
        __caption__: ''
      },
      attributes: {
        __caption__: '',
        accessModifier: {
          __caption__: ''
        },
        name: {
          __caption__: ''
        },
        description: {
          __caption__: ''
        },
        type: {
          __caption__: ''
        },
        defaultValue: {
          __caption__: ''
        },
        pBCustomAttributes: {
          __caption__: ''
        },
        pBGetEnd: {
          __caption__: ''
        },
        pBGetStart: {
          __caption__: ''
        },
        pBSetEnd: {
          __caption__: ''
        },
        pBSetStart: {
          __caption__: ''
        }
      },
      formViews: {
        __caption__: '',
        viewForForm: {
          __caption__: ''
        },
        view: {
          __caption__: ''
        }
      },
      methods: {
        __caption__: '',
        accessModifier: {
          __caption__: ''
        },
        name: {
          __caption__: ''
        },
        caption: {
          __caption__: ''
        },
        description: {
          __caption__: ''
        },
        type: {
          __caption__: ''
        },
        parametersStr: {
          __caption__: ''
        },
        pBCustomAttributes: {
          __caption__: ''
        },
        publishToUser: {
          __caption__: ''
        },
        isEvent: {
          __caption__: ''
        },
        accessType: {
          __caption__: ''
        },
        parameters: {
          __caption__: '',
          name: {
          __caption__: ''
        },
          modifier: {
          __caption__: ''
        },
          type: {
          __caption__: ''
        },
          description: {
          __caption__: ''
        }
        }
      }
    },
    EditEnum: {
      name: {
        __caption__: ''
      },
      description: {
        __caption__: ''
      },
      onlyShowSelectedValue: {
        __caption__: ''
      },
      packet: {
        __caption__: ''
      },
      namespacePostfix: {
        __caption__: ''
      },
      attributes: {
        __caption__: '',
        name: {
          __caption__: ''
        },
        description: {
          __caption__: ''
        },
        defaultValue: {
          __caption__: ''
        },
        caption: {
          __caption__: ''
        }
      }
    },
    EditEventArg: {
      name: {
        __caption__: ''
      },
      description: {
        __caption__: ''
      },
      generateCatcher: {
        __caption__: ''
      },
      packet: {
        __caption__: ''
      },
      namespacePostfix: {
        __caption__: ''
      },
      pBMembers: {
        __caption__: ''
      },
      attributes: {
        __caption__: '',
        accessModifier: {
          __caption__: ''
        },
        name: {
          __caption__: ''
        },
        description: {
          __caption__: ''
        },
        type: {
          __caption__: ''
        },
        defaultValue: {
          __caption__: ''
        },
        pBCustomAttributes: {
          __caption__: ''
        },
        pBGetEnd: {
          __caption__: ''
        },
        pBGetStart: {
          __caption__: ''
        },
        pBSetEnd: {
          __caption__: ''
        },
        pBSetStart: {
          __caption__: ''
        }
      },
      methods: {
        __caption__: '',
        accessModifier: {
          __caption__: ''
        },
        name: {
          __caption__: ''
        },
        description: {
          __caption__: ''
        },
        type: {
          __caption__: ''
        },
        parametersStr: {
          __caption__: ''
        },
        isEvent: {
          __caption__: ''
        },
        pBCustomAttributes: {
          __caption__: ''
        },
        parameters: {
          __caption__: '',
          name: {
          __caption__: ''
        },
          modifier: {
          __caption__: ''
        },
          type: {
          __caption__: ''
        },
          description: {
          __caption__: ''
        }
        }
      }
    },
    EditExternal: {
      name: {
        __caption__: ''
      },
      nameStr: {
        __caption__: ''
      },
      description: {
        __caption__: 'Template class'
      },
      storeInstancesInTypeXML: {
        __caption__: 'StoreInstancesInType'
      },
      packet: {
        __caption__: ''
      },
      namespacePostfix: {
        __caption__: ''
      },
      pBMembers: {
        __caption__: ''
      },
      pBCustomAttributes: {
        __caption__: ''
      },
      publishToEBSD: {
        __caption__: ''
      },
      stage: {
        __caption__: '',
        name: {
          __caption__: ''
        },
        configuration: {
          __caption__: '',
          name: {
            __caption__: ''
          },
          project: {
            __caption__: '',
            name: {
              __caption__: ''
            },
            repository: {
              __caption__: '',
              name: {
                __caption__: ''
              }
            }
          }
        }
      }
    },
    EditFormView: {
      name: {
        __caption__: 'Name'
      },
      description: {
        __caption__: 'Description'
      }
    },
    EditInterface: {
      bSClass: {
        __caption__: 'Business Server'
      },
      bSEvents: {
        __caption__: 'BS Events'
      },
      businessServerClass: {
        __caption__: '',
        stereotype: {
          __caption__: ''
        }
      },
      description: {
        __caption__: ''
      },
      name: {
        __caption__: ''
      },
      namespacePostfix: {
        __caption__: ''
      },
      packet: {
        __caption__: ''
      },
      pBMembers: {
        __caption__: ''
      },
      publishToEBSD: {
        __caption__: ''
      },
      attributes: {
        __caption__: '',
        name: {
          __caption__: ''
        },
        type: {
          __caption__: ''
        },
        description: {
          __caption__: ''
        }
      },
      methods: {
        __caption__: '',
        accessModifier: {
          __caption__: ''
        },
        name: {
          __caption__: ''
        },
        description: {
          __caption__: ''
        },
        type: {
          __caption__: ''
        },
        parametersStr: {
          __caption__: ''
        },
        isEvent: {
          __caption__: ''
        },
        pBCustomAttributes: {
          __caption__: ''
        },
        parameters: {
          __caption__: '',
          name: {
          __caption__: ''
        },
          modifier: {
          __caption__: ''
        },
          type: {
          __caption__: ''
        },
          description: {
          __caption__: ''
        }
        }
      }
    },
    EditListForm: {
      name: {
        __caption__: ''
      },
      description: {
        __caption__: ''
      },
      caption: {
        __caption__: ''
      },
      generateDependedForm: {
        __caption__: ''
      },
      fixDependedForm: {
        __caption__: ''
      },
      packet: {
        __caption__: ''
      },
      namespacePostfix: {
        __caption__: ''
      },
      pBCustomAttributes: {
        __caption__: ''
      },
      pBMembers: {
        __caption__: ''
      },
      scriptName: {
        __caption__: ''
      },
      publishToEBSD: {
        __caption__: ''
      },
      attributes: {
        __caption__: '',
        accessModifier: {
          __caption__: ''
        },
        name: {
          __caption__: ''
        },
        description: {
          __caption__: ''
        },
        type: {
          __caption__: ''
        },
        defaultValue: {
          __caption__: ''
        },
        pBCustomAttributes: {
          __caption__: ''
        },
        pBGetEnd: {
          __caption__: ''
        },
        pBGetStart: {
          __caption__: ''
        },
        pBSetEnd: {
          __caption__: ''
        },
        pBSetStart: {
          __caption__: ''
        }
      },
      formViews: {
        __caption__: '',
        viewForForm: {
          __caption__: ''
        },
        dataObjectTypes: {
          __caption__: ''
        },
        listFormOperations: {
          __caption__: ''
        },
        hierarchicalMaster: {
          __caption__: ''
        },
        view: {
          __caption__: ''
        }
      },
      methods: {
        __caption__: '',
        accessModifier: {
          __caption__: ''
        },
        name: {
          __caption__: ''
        },
        caption: {
          __caption__: ''
        },
        description: {
          __caption__: ''
        },
        type: {
          __caption__: ''
        },
        parametersStr: {
          __caption__: ''
        },
        pBCustomAttributes: {
          __caption__: ''
        },
        publishToUser: {
          __caption__: ''
        },
        isEvent: {
          __caption__: ''
        },
        accessType: {
          __caption__: ''
        },
        parameters: {
          __caption__: '',
          name: {
          __caption__: ''
        },
          modifier: {
          __caption__: ''
        },
          type: {
          __caption__: ''
        },
          description: {
          __caption__: ''
        }
        }
      }
    },
    EditPrintForm: {
      name: {
        __caption__: ''
      },
      description: {
        __caption__: ''
      },
      caption: {
        __caption__: ''
      },
      generateDependedForm: {
        __caption__: ''
      },
      packet: {
        __caption__: ''
      },
      namespacePostfix: {
        __caption__: ''
      },
      pBCustomAttributes: {
        __caption__: ''
      },
      pBGetViewsForForm: {
        __caption__: ''
      },
      pBMembers: {
        __caption__: ''
      },
      publishToEBSD: {
        __caption__: ''
      },
      attributes: {
        __caption__: '',
        accessModifier: {
          __caption__: ''
        },
        name: {
          __caption__: ''
        },
        description: {
          __caption__: ''
        },
        type: {
          __caption__: ''
        },
        defaultValue: {
          __caption__: ''
        },
        pBCustomAttributes: {
          __caption__: ''
        },
        pBGetEnd: {
          __caption__: ''
        },
        pBGetStart: {
          __caption__: ''
        },
        pBSetEnd: {
          __caption__: ''
        },
        pBSetStart: {
          __caption__: ''
        }
      },
      formViews: {
        __caption__: '',
        viewForForm: {
          __caption__: ''
        },
        view: {
          __caption__: ''
        }
      },
      methods: {
        __caption__: '',
        accessModifier: {
          __caption__: ''
        },
        name: {
          __caption__: ''
        },
        caption: {
          __caption__: ''
        },
        description: {
          __caption__: ''
        },
        type: {
          __caption__: ''
        },
        parametersStr: {
          __caption__: ''
        },
        pBCustomAttributes: {
          __caption__: ''
        },
        isEvent: {
          __caption__: ''
        },
        parameters: {
          __caption__: '',
          name: {
          __caption__: ''
        },
          modifier: {
          __caption__: ''
        },
          type: {
          __caption__: ''
        },
          description: {
          __caption__: ''
        }
        }
      }
    },
    EditPropertyLookups: {
      name: {
        __caption__: ''
      },
      nameStr: {
        __caption__: ''
      },
      stereotype: {
        __caption__: ''
      },
      formViews: {
        __caption__: '',
        view: {
          __caption__: '',
          name: {
            __caption__: ''
          },
          class: {
            __caption__: '',
            name: {
              __caption__: ''
            }
          }
        }
      },
      views: {
        __caption__: '',
        name: {
          __caption__: ''
        },
        definition: {
          __caption__: ''
        },
        class: {
          __caption__: '',
          name: {
            __caption__: ''
          }
        }
      }
    },
    EditType: {
      name: {
        __caption__: ''
      },
      description: {
        __caption__: ''
      },
      storeInstancesInTypeXML: {
        __caption__: 'StoreInstancesInType'
      },
      packet: {
        __caption__: ''
      },
      namespacePostfix: {
        __caption__: ''
      },
      pBMembers: {
        __caption__: ''
      },
      pBCustomAttributes: {
        __caption__: ''
      },
      publishToEBSD: {
        __caption__: ''
      },
      attributes: {
        __caption__: '',
        accessModifier: {
          __caption__: ''
        },
        name: {
          __caption__: ''
        },
        description: {
          __caption__: ''
        },
        type: {
          __caption__: ''
        },
        defaultValue: {
          __caption__: ''
        },
        pBCustomAttributes: {
          __caption__: ''
        },
        pBGetEnd: {
          __caption__: ''
        },
        pBGetStart: {
          __caption__: ''
        },
        pBSetEnd: {
          __caption__: ''
        },
        pBSetStart: {
          __caption__: ''
        }
      },
      methods: {
        __caption__: '',
        accessModifier: {
          __caption__: ''
        },
        name: {
          __caption__: ''
        },
        description: {
          __caption__: ''
        },
        type: {
          __caption__: ''
        },
        parametersStr: {
          __caption__: ''
        },
        isEvent: {
          __caption__: ''
        },
        pBCustomAttributes: {
          __caption__: ''
        },
        parameters: {
          __caption__: '',
          name: {
          __caption__: ''
        },
          modifier: {
          __caption__: ''
        },
          type: {
          __caption__: ''
        },
          description: {
          __caption__: ''
        }
        }
      }
    },
    EditUserForm: {
      name: {
        __caption__: ''
      },
      description: {
        __caption__: ''
      },
      pBCustomAttributes: {
        __caption__: ''
      },
      pBMembers: {
        __caption__: ''
      },
      packet: {
        __caption__: ''
      },
      namespacePostfix: {
        __caption__: ''
      },
      scriptName: {
        __caption__: ''
      },
      publishToEBSD: {
        __caption__: ''
      },
      attributes: {
        __caption__: '',
        accessModifier: {
          __caption__: ''
        },
        name: {
          __caption__: ''
        },
        description: {
          __caption__: ''
        },
        type: {
          __caption__: ''
        },
        defaultValue: {
          __caption__: ''
        },
        pBCustomAttributes: {
          __caption__: ''
        },
        pBGetEnd: {
          __caption__: ''
        },
        pBGetStart: {
          __caption__: ''
        },
        pBSetEnd: {
          __caption__: ''
        },
        pBSetStart: {
          __caption__: ''
        }
      },
      methods: {
        __caption__: '',
        accessModifier: {
          __caption__: ''
        },
        name: {
          __caption__: ''
        },
        caption: {
          __caption__: ''
        },
        description: {
          __caption__: ''
        },
        type: {
          __caption__: ''
        },
        parametersStr: {
          __caption__: ''
        },
        pBCustomAttributes: {
          __caption__: ''
        },
        publishToUser: {
          __caption__: ''
        },
        isEvent: {
          __caption__: ''
        },
        accessType: {
          __caption__: ''
        },
        parameters: {
          __caption__: '',
          name: {
          __caption__: ''
        },
          modifier: {
          __caption__: ''
        },
          type: {
          __caption__: ''
        },
          description: {
          __caption__: ''
        }
        }
      }
    },
    FdAttributesChangeView: {
      attributesStr: {
        __caption__: ''
      },
      name: {
        __caption__: ''
      },
      nameStr: {
        __caption__: ''
      },
      stereotype: {
        __caption__: ''
      },
      attributes: {
        __caption__: '',
        name: {
          __caption__: ''
        },
        type: {
          __caption__: ''
        },
        caption: {
          __caption__: ''
        },
        description: {
          __caption__: ''
        },
        class: {
          __caption__: ''
        }
      },
      views: {
        __caption__: '',
        name: {
          __caption__: ''
        },
        definition: {
          __caption__: ''
        },
        class: {
          __caption__: ''
        }
      }
    },
    FdAttributesForForm: {
      name: {
        __caption__: ''
      },
      stereotype: {
        __caption__: ''
      },
      formViews: {
        __caption__: '',
        dataObjectTypesStr: {
          __caption__: ''
        },
        view: {
          __caption__: '',
          class: {
            __caption__: '',
            name: {
              __caption__: ''
            }
          },
          definition: {
            __caption__: ''
          }
        },
        class: {
          __caption__: ''
        }
      }
    },
    FdEditClassForm: {
      caption: {
        __caption__: 'Заголовок'
      },
      name: {
        __caption__: 'Имя'
      },
      publishName: {
        __caption__: 'Алиас'
      },
      stored: {
        __caption__: 'Хранимый'
      },
      storage: {
        __caption__: 'Хранилище'
      },
      packet: {
        __caption__: 'Пакет'
      },
      namespacePostfix: {
        __caption__: 'Постфикс пространства имен'
      },
      //bSClass: {
      //  __caption__: 'Класс БС'
      //},
      attributes: {
        __caption__: 'Атрибуты',
        name: {
          __caption__: 'Имя'
        },
        caption: {
          __caption__: 'Заголовок'
        },
        type: {
          __caption__: 'Тип'
        },
        description: {
          __caption__: 'Описание'
        },
        class: {
          __caption__: 'Класс'
        }
      },
      views: {
        __caption__: 'Представления',
        name: {
          __caption__: 'Имя'
        },
        description: {
          __caption__: 'Описание'
        },
        class: {
          __caption__: 'Класс'
        }
      },
      classStorageTypes: {
        __caption__: 'Типы хранилищ',
        connectionName: {
          __caption__: 'Имя соединения'
        },
        connectionString: {
          __caption__: 'Строка соединения'
        },
        class: {
          __caption__: 'Класс'
        },
        storageType: {
          __caption__: 'Тип хранилища'
        }
      }
    },
    Generator: {
      referenceCount: {
        __caption__: ''
      },
      stored: {
        __caption__: ''
      },
      attributesStr: {
        __caption__: ''
      },
      methodsStr: {
        __caption__: ''
      },
      name: {
        __caption__: ''
      },
      nameStr: {
        __caption__: ''
      },
      stereotype: {
        __caption__: ''
      },
      pBMembers: {
        __caption__: ''
      },
      autoAltered: {
        __caption__: ''
      },
      caption: {
        __caption__: ''
      },
      loadingOrder: {
        __caption__: ''
      },
      onlyShowSelectedValue: {
        __caption__: ''
      },
      pBCustomAttributes: {
        __caption__: ''
      },
      primaryKeyStorage: {
        __caption__: ''
      },
      storage: {
        __caption__: ''
      },
      publishName: {
        __caption__: ''
      },
      storeInstancesInType: {
        __caption__: ''
      },
      trim: {
        __caption__: ''
      },
      description: {
        __caption__: ''
      },
      generateDependedForm: {
        __caption__: ''
      },
      fixDependedForm: {
        __caption__: ''
      },
      packet: {
        __caption__: ''
      },
      namespacePostfix: {
        __caption__: ''
      },
      pBGetViewsForForm: {
        __caption__: ''
      },
      businessServerEvents: {
        __caption__: ''
      },
      businessServerClass: {
        __caption__: '',
        nameStr: {
          __caption__: ''
        },
        attributesStr: {
          __caption__: ''
        },
        methodsStr: {
          __caption__: ''
        }
      },
      disableAllRightChecks: {
        __caption__: ''
      },
      dataObjectTypesStr: {
        __caption__: ''
      },
      propertyLookupStr: {
        __caption__: ''
      },
      standartDesktop: {
        __caption__: ''
      },
      containersStr: {
        __caption__: ''
      },
      appConfig: {
        __caption__: ''
      },
      hierarchicalMaster: {
        __caption__: ''
      },
      listFormOperationsStr: {
        __caption__: ''
      },
      editFormOperationsStr: {
        __caption__: ''
      },
      printContainer: {
        __caption__: ''
      },
      generateComPlusServer: {
        __caption__: ''
      },
      comPlusServerPropertiesStr: {
        __caption__: ''
      },
      generateHttpRemoteServer: {
        __caption__: ''
      },
      scriptName: {
        __caption__: ''
      },
      publishToEBSD: {
        __caption__: ''
      },
      generateCatcher: {
        __caption__: ''
      },
      expandOperations: {
        __caption__: ''
      },
      accessType: {
        __caption__: ''
      },
      auditEnabled: {
        __caption__: ''
      },
      useDefaultView: {
        __caption__: ''
      },
      insertAudit: {
        __caption__: ''
      },
      insertAuditViewName: {
        __caption__: ''
      },
      deleteAudit: {
        __caption__: ''
      },
      deleteAuditViewName: {
        __caption__: ''
      },
      updateAudit: {
        __caption__: ''
      },
      updateAuditViewName: {
        __caption__: ''
      },
      selectAudit: {
        __caption__: ''
      },
      selectAuditViewName: {
        __caption__: ''
      },
      formUrl: {
        __caption__: ''
      },
      writeMode: {
        __caption__: ''
      },
      auditConnectionStringName: {
        __caption__: ''
      },
      auditWinServiceUrl: {
        __caption__: ''
      },
      addAuditFields: {
        __caption__: ''
      },
      writeSessions: {
        __caption__: ''
      },
      attributes: {
        __caption__: '',
        accessModifier: {
          __caption__: 'AccessModifier'
        },
        autoincrement: {
          __caption__: 'Autoincrement'
        },
        caption: {
          __caption__: 'Caption'
        },
        dataServiceExpression: {
          __caption__: 'DataServiceExpression'
        },
        dataServiceExpressionXML: {
          __caption__: 'DataServiceExpressionXML'
        },
        defaultValue: {
          __caption__: 'DefaultValue'
        },
        hint: {
          __caption__: 'Hint'
        },
        notNull: {
          __caption__: 'NotNull'
        },
        order: {
          __caption__: 'Order'
        },
        orderNum: {
          __caption__: 'OrderNum'
        },
        pBCustomAttributes: {
          __caption__: 'PBCustomAttributes'
        },
        pBGetEnd: {
          __caption__: 'PBGetEnd'
        },
        pBSetEnd: {
          __caption__: 'PBSetEnd'
        },
        pBGetStart: {
          __caption__: 'PBGetStart'
        },
        pBSetStart: {
          __caption__: 'PBSetStart'
        },
        realCaption: {
          __caption__: 'RealCaption'
        },
        realStorage: {
          __caption__: 'RealStorage'
        },
        storage: {
          __caption__: 'Storage'
        },
        publishName: {
          __caption__: 'PublishName'
        },
        stored: {
          __caption__: 'Stored'
        },
        trim: {
          __caption__: 'Trim'
        },
        type: {
          __caption__: 'Type'
        },
        name: {
          __caption__: 'Name'
        },
        description: {
          __caption__: 'Description'
        },
        nameStr: {
          __caption__: 'NameStr'
        }
      },
      methods: {
        __caption__: '',
        accessModifier: {
          __caption__: 'AccessModifier'
        },
        accessType: {
          __caption__: 'AccessType'
        },
        caption: {
          __caption__: 'Caption'
        },
        isEvent: {
          __caption__: 'IsEvent'
        },
        orderNum: {
          __caption__: 'OrderNum'
        },
        parametersStr: {
          __caption__: 'ParametersStr'
        },
        pBCustomAttributes: {
          __caption__: 'PBCustomAttributes'
        },
        publishToUser: {
          __caption__: 'PublishToUser'
        },
        realCaption: {
          __caption__: 'RealCaption'
        },
        type: {
          __caption__: 'Type'
        },
        typeParametersStr: {
          __caption__: 'TypeParametersStr'
        },
        name: {
          __caption__: 'Name'
        },
        description: {
          __caption__: 'Description'
        },
        nameStr: {
          __caption__: 'NameStr'
        },
        parameters: {
          __caption__: '',
          caption: {
          __caption__: 'Caption'
        },
          modifier: {
          __caption__: 'Modifier'
        },
          orderNum: {
          __caption__: 'OrderNum'
        },
          realCaption: {
          __caption__: 'RealCaption'
        },
          type: {
          __caption__: 'Type'
        },
          name: {
          __caption__: 'Name'
        },
          description: {
          __caption__: 'Description'
        },
          nameStr: {
          __caption__: 'NameStr'
        }
        }
      },
      formViews: {
        __caption__: '',
        view: {
          __caption__: '',
          name: {
            __caption__: ''
          },
          class: {
            __caption__: '',
            name: {
              __caption__: ''
            }
          }
        },
        dataObjectTypes: {
          __caption__: 'DataObjectTypes'
        },
        dataObjectTypesStr: {
          __caption__: 'DataObjectTypesStr'
        },
        hierarchicalMaster: {
          __caption__: 'HierarchicalMaster'
        },
        listFormOperations: {
          __caption__: 'ListFormOperations'
        },
        listFormOperationsStr: {
          __caption__: 'ListFormOperationsStr'
        },
        orderNum: {
          __caption__: 'OrderNum'
        },
        propertyLookup: {
          __caption__: 'PropertyLookup'
        },
        propertyLookupStr: {
          __caption__: 'PropertyLookupStr'
        },
        viewForForm: {
          __caption__: 'ViewForForm'
        },
        name: {
          __caption__: 'Name'
        },
        description: {
          __caption__: 'Description'
        },
        nameStr: {
          __caption__: 'NameStr'
        },
        controls: {
          __caption__: '',
          name: {
          __caption__: ''
        },
          propertyPath: {
          __caption__: ''
        },
          settingsXml: {
          __caption__: ''
        },
          order: {
          __caption__: ''
        },
          controlType: {
            __caption__: '',
            name: {
              __caption__: ''
            }
          },
          propertyType: {
            __caption__: '',
            name: {
              __caption__: ''
            }
          },
          formView: {
            __caption__: '',
            view: {
              __caption__: ''
            }
          }
        }
      },
      views: {
        __caption__: '',
        name: {
          __caption__: ''
        },
        description: {
          __caption__: ''
        },
        definition: {
          __caption__: ''
        },
        class: {
          __caption__: ''
        }
      },
      classStorageTypes: {
        __caption__: 'Типы хранилищ',
        connectionName: {
          __caption__: 'Имя соединения'
        },
        connectionString: {
          __caption__: 'Строка соединения'
        },
        storageType: {
          __caption__: 'Тип хранилища'
        }
      }
    },
    GetClassByGuid: {
      name: {
        __caption__: ''
      },
      description: {
        __caption__: ''
      },
      stereotype: {
        __caption__: ''
      },
      caption: {
        __caption__: ''
      },
      attributesStr: {
        __caption__: ''
      },
      stage: {
        __caption__: ''
      }
    },
    LightStage: {
      generateCatcher: {
        __caption__: ''
      },
      generateComPlusServer: {
        __caption__: ''
      },
      generateHttpRemoteServer: {
        __caption__: ''
      },
      nameStr: {
        __caption__: ''
      },
      packet: {
        __caption__: ''
      },
      stereotype: {
        __caption__: ''
      }
    },
    ListDataObjectTypes: {
      name: {
        __caption__: ''
      },
      nameStr: {
        __caption__: ''
      },
      stereotype: {
        __caption__: ''
      },
      formViews: {
        __caption__: '',
        view: {
          __caption__: '',
          name: {
            __caption__: ''
          },
          class: {
            __caption__: '',
            name: {
              __caption__: ''
            }
          }
        }
      }
    },
    ListFormView: {
      referenceCount: {
        __caption__: 'Reference count'
      },
      stereotype: {
        __caption__: 'Stereotype'
      },
      name: {
        __caption__: 'Name'
      },
      description: {
        __caption__: 'Description'
      },
      packet: {
        __caption__: 'Packet'
      },
      namespacePostfix: {
        __caption__: 'Namespace postfix'
      },
      stage: {
        __caption__: ''
      }
    },
    LoadNameView: {
      name: {
        __caption__: ''
      },
      nameStr: {
        __caption__: ''
      },
      stereotype: {
        __caption__: ''
      },
      description: {
        __caption__: ''
      },
      caption: {
        __caption__: ''
      }
    },
    Prototyping: {
      stereotype: {
        __caption__: ''
      },
      nameStr: {
        __caption__: ''
      },
      views: {
        __caption__: '',
        name: {
          __caption__: ''
        },
        definition: {
          __caption__: ''
        }
      }
    },
    SearchAppClassAndEditContainers: {
      name: {
        __caption__: ''
      },
      nameStr: {
        __caption__: ''
      },
      stereotype: {
        __caption__: ''
      },
      caption: {
        __caption__: ''
      },
      containersStr: {
        __caption__: ''
      },
      stage: {
        __caption__: ''
      }
    },
    SearchClass: {
      name: {
        __caption__: ''
      },
      nameStr: {
        __caption__: ''
      },
      stereotype: {
        __caption__: ''
      },
      caption: {
        __caption__: ''
      },
      stage: {
        __caption__: ''
      }
    },
    SearchClassLoadView: {
      name: {
        __caption__: ''
      },
      nameStr: {
        __caption__: ''
      },
      stereotype: {
        __caption__: ''
      },
      caption: {
        __caption__: ''
      },
      stage: {
        __caption__: ''
      },
      formViews: {
        __caption__: '',
        dataObjectTypesStr: {
          __caption__: ''
        },
        view: {
          __caption__: '',
          class: {
            __caption__: '',
            name: {
              __caption__: ''
            }
          },
          definition: {
            __caption__: ''
          }
        },
        class: {
          __caption__: ''
        }
      }
    },
    SearchFormClassView: {
      name: {
        __caption__: ''
      },
      nameStr: {
        __caption__: ''
      },
      stereotype: {
        __caption__: ''
      },
      containersStr: {
        __caption__: ''
      },
      stage: {
        __caption__: ''
      }
    },
    SearchListFormAndEditLink: {
      name: {
        __caption__: ''
      },
      nameStr: {
        __caption__: ''
      },
      stereotype: {
        __caption__: ''
      },
      caption: {
        __caption__: ''
      },
      dataObjectTypesStr: {
        __caption__: ''
      },
      stage: {
        __caption__: ''
      },
      formViews: {
        __caption__: '',
        dataObjectTypesStr: {
          __caption__: ''
        },
        view: {
          __caption__: '',
          class: {
            __caption__: '',
            name: {
              __caption__: ''
            }
          },
          definition: {
            __caption__: ''
          }
        },
        class: {
          __caption__: ''
        }
      }
    },
    ViewPeeker: {
      name: {
        __caption__: ''
      },
      nameStr: {
        __caption__: ''
      },
      stereotype: {
        __caption__: ''
      },
      description: {
        __caption__: ''
      },
      views: {
        __caption__: '',
        name: {
          __caption__: ''
        }
      }
    }
  },
  validations: {
    caption: 'Введите заголовок'
  }
};

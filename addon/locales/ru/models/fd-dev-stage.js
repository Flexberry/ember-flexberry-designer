export default {
  projections: {
    AuditPrototyping: {
      typeMapCS: {
        __caption__: ''
      },
      typeMapCSStr: {
        __caption__: ''
      }
    },
    Backup: {
      name: {
        __caption__: ''
      },
      description: {
        __caption__: ''
      },
      company: {
        __caption__: ''
      },
      product: {
        __caption__: ''
      },
      copyright: {
        __caption__: ''
      },
      version: {
        __caption__: ''
      },
      dataObjectNameSpace: {
        __caption__: ''
      },
      sourceCodeCSPath: {
        __caption__: ''
      },
      sourceCodeVBPath: {
        __caption__: ''
      },
      sQLPath: {
        __caption__: ''
      },
      typeMapCSStr: {
        __caption__: ''
      },
      typeMapVBStr: {
        __caption__: ''
      },
      typeMapSQLStr: {
        __caption__: ''
      },
      typeMapAccessStr: {
        __caption__: ''
      },
      typeMapOracleStr: {
        __caption__: ''
      },
      defaultBaseClass: {
        __caption__: ''
      },
      defaultDetailArrayClass: {
        __caption__: ''
      },
      connectionString: {
        __caption__: ''
      },
      oracleConnectionString: {
        __caption__: ''
      },
      postgreConnectionString: {
        __caption__: ''
      },
      additionalPluginsSettingsStr: {
        __caption__: ''
      },
      defaultEditScriptName: {
        __caption__: ''
      },
      defaultListScriptName: {
        __caption__: ''
      },
      sourceAzStoragePath: {
        __caption__: ''
      },
      operationsEnumPacket: {
        __caption__: ''
      },
      operationsEnumNamespace: {
        __caption__: ''
      },
      signAssemblies: {
        __caption__: ''
      },
      auditEnabled: {
        __caption__: ''
      },
      isAuditDatabaseLocal: {
        __caption__: ''
      },
      isReportDatabaseLocal: {
        __caption__: ''
      },
      defaultWriteMode: {
        __caption__: ''
      },
      moduleSettings: {
        __caption__: '',
        valueXML: {
          __caption__: ''
        },
        moduleSettingTypeName: {
          __caption__: ''
        },
        moduleSettingType: {
          __caption__: ''
        },
        stage: {
          __caption__: ''
        }
      }
    },
    Compiler: {
      company: {
        __caption__: ''
      },
      product: {
        __caption__: ''
      },
      copyright: {
        __caption__: ''
      },
      version: {
        __caption__: ''
      },
      dataObjectNameSpace: {
        __caption__: ''
      },
      sourceCodeCSPath: {
        __caption__: ''
      },
      sourceCodeVBPath: {
        __caption__: ''
      },
      sQLPath: {
        __caption__: ''
      },
      typeMapCSStr: {
        __caption__: ''
      },
      typeMapVBStr: {
        __caption__: ''
      },
      typeMapSQLStr: {
        __caption__: ''
      },
      typeMapAccessStr: {
        __caption__: ''
      },
      defaultBaseClass: {
        __caption__: ''
      },
      defaultDetailArrayClass: {
        __caption__: ''
      },
      defaultEditScriptName: {
        __caption__: ''
      },
      defaultListScriptName: {
        __caption__: ''
      },
      signAssemblies: {
        __caption__: ''
      },
      additionalPluginsSettingsStr: {
        __caption__: ''
      },
      name: {
        __caption__: ''
      },
      description: {
        __caption__: ''
      }
    },
    EditAccessGenerator: {
      name: {
        __caption__: 'Название стадии'
      },
      description: {
        __caption__: 'Описание'
      },
      sQLPath: {
        __caption__: ''
      },
      typeMapAccessStr: {
        __caption__: ''
      },
      typeMapAccess: {
        __caption__: 'Карта типов'
      },
      additionalPluginsSettings: {
        __caption__: 'Дополнительные настройки'
      },
      additionalPluginsSettingsStr: {
        __caption__: ''
      },
      moduleSettings: {
        __caption__: '',
        moduleSettingType: {
          __caption__: 'ModuleSettingTyp',
          name: {
            __caption__: 'Название ModuleSettingType'
          }
        },
        valueXML: {
          __caption__: 'ValueXML'
        },
        moduleSettingTypeName: {
          __caption__: ''
        },
        stage: {
          __caption__: 'Stage'
        }
      }
    },
    EditCSGenerator: {
      name: {
        __caption__: 'Название стадии'
      },
      description: {
        __caption__: 'Описание'
      },
      company: {
        __caption__: 'Название компании'
      },
      product: {
        __caption__: 'Название продукта'
      },
      copyright: {
        __caption__: 'Copyright'
      },
      version: {
        __caption__: 'Версия'
      },
      sourceCodeCSPath: {
        __caption__: 'Каталог для исходного кода'
      },
      typeMapCSStr: {
        __caption__: ''
      },
      typeMapCS: {
        __caption__: 'Карта типов'
      },
      enableAuElement: {
        __caption__: ''
      },
      auditEnabled: {
        __caption__: 'Вести аудит в приложение'
      },
      defaultWriteMode: {
        __caption__: 'Режим записи аудита по умолчанию'
      },
      dataObjectNameSpace: {
        __caption__: 'NameSpace'
      },
      defaultBaseClass: {
        __caption__: 'Default base class'
      },
      defaultDetailArrayClass: {
        __caption__: 'Default detail array class'
      },
      defaultEditScriptName: {
        __caption__: 'Default edit script name'
      },
      defaultListScriptName: {
        __caption__: 'Default list script name'
      },
      scriptPacket: {
        __caption__: 'Script packet'
      },
      scriptNamespace: {
        __caption__: 'Script namespace postfix'
      },
      signAssemblies: {
        __caption__: 'Подписывать сборки'
      },
      additionalPluginsSettings: {
        __caption__: 'Дополнительные настройки'
      },
      additionalPluginsSettingsStr: {
        __caption__: ''
      },
      moduleSettings: {
        __caption__: '',
        moduleSettingType: {
          __caption__: 'ModuleSettingTyp',
          name: {
            __caption__: 'Название ModuleSettingType'
          }
        },
        valueXML: {
          __caption__: 'ValueXML'
        },
        moduleSettingTypeName: {
          __caption__: ''
        },
        stage: {
          __caption__: 'Stage'
        }
      }
    },
    EditFormView: {
      name: {
        __caption__: 'Название'
      },
      description: {
        __caption__: 'Описание'
      },
      company: {
        __caption__: 'Company'
      },
      copyright: {
        __caption__: 'Copyright'
      },
      product: {
        __caption__: 'Product'
      },
      configuration: {
        __caption__: '',
        project: {
          __caption__: '',
          repository: {
            __caption__: ''
          }
        }
      }
    },
    EditMSSQLDirectGenerator: {
      name: {
        __caption__: 'Название стадии'
      },
      description: {
        __caption__: 'Описание'
      },
      typeMapSQLStr: {
        __caption__: ''
      },
      typeMapSQL: {
        __caption__: 'Карта типов'
      },
      connectionString: {
        __caption__: 'Строка соединения'
      },
      additionalPluginsSettings: {
        __caption__: 'Дополнительные настройки'
      },
      additionalPluginsSettingsStr: {
        __caption__: ''
      },
      doNotDeleteExtraTables: {
        __caption__: 'Не удалять существующие таблицы'
      },
      isAuditDatabaseLocal: {
        __caption__: 'БД аудита в БД приложения'
      },
      isReportDatabaseLocal: {
        __caption__: 'БД web-отчётов в БД приложения'
      },
      moduleSettings: {
        __caption__: '',
        moduleSettingType: {
          __caption__: 'ModuleSettingTyp',
          name: {
            __caption__: 'Название ModuleSettingType'
          }
        },
        valueXML: {
          __caption__: 'ValueXML'
        },
        moduleSettingTypeName: {
          __caption__: ''
        },
        stage: {
          __caption__: 'Stage'
        }
      }
    },
    EditOracleGenerator: {
      name: {
        __caption__: 'Название стадии'
      },
      description: {
        __caption__: 'Описание'
      },
      sQLPath: {
        __caption__: ''
      },
      typeMapOracle: {
        __caption__: 'Карта типов'
      },
      doNotDeleteExtraTables: {
        __caption__: 'Не удалять существующие таблицы'
      },
      typeMapOracleStr: {
        __caption__: ''
      },
      isAuditDatabaseLocal: {
        __caption__: 'БД аудита в БД приложения'
      },
      isReportDatabaseLocal: {
        __caption__: 'БД web-отчётов в БД приложения'
      },
      additionalPluginsSettings: {
        __caption__: 'Дополнительные настройки'
      },
      additionalPluginsSettingsStr: {
        __caption__: ''
      },
      oracleConnectionString: {
        __caption__: 'Строка соединения'
      },
      moduleSettings: {
        __caption__: '',
        moduleSettingType: {
          __caption__: 'ModuleSettingTyp',
          name: {
            __caption__: 'Название ModuleSettingType'
          }
        },
        valueXML: {
          __caption__: 'ValueXML'
        },
        moduleSettingTypeName: {
          __caption__: ''
        },
        stage: {
          __caption__: 'Stage'
        }
      }
    },
    EditPostgreSqlDirectGenerator: {
      name: {
        __caption__: 'Название стадии'
      },
      description: {
        __caption__: 'Описание'
      },
      typeMapPostgre: {
        __caption__: 'Карта типов'
      },
      postgreConnectionString: {
        __caption__: 'Строка соединения'
      },
      typeMapPostgreStr: {
        __caption__: ''
      },
      additionalPluginsSettingsStr: {
        __caption__: ''
      },
      doNotDeleteExtraTables: {
        __caption__: 'Не удалять существующие таблицы'
      },
      additionalPluginsSettings: {
        __caption__: 'Дополнительные настройки'
      },
      isAuditDatabaseLocal: {
        __caption__: 'БД аудита в БД приложения'
      },
      isReportDatabaseLocal: {
        __caption__: 'БД web-отчётов в БД приложения'
      },
      moduleSettings: {
        __caption__: '',
        moduleSettingType: {
          __caption__: 'ModuleSettingTyp',
          name: {
            __caption__: 'Название ModuleSettingType'
          }
        },
        valueXML: {
          __caption__: 'ValueXML'
        },
        moduleSettingTypeName: {
          __caption__: ''
        },
        stage: {
          __caption__: 'Stage'
        }
      }
    },
    EditPropertyLookups: {
    },
    EditRightManagementGenerator: {
      name: {
        __caption__: 'Название стадии'
      },
      description: {
        __caption__: 'Описание'
      },
      sourceAzStoragePath: {
        __caption__: 'Имя файла хранилища'
      },
      defaultAccessType: {
        __caption__: 'Тип проверки доступа по умолчанию'
      },
      additionalPluginsSettingsStr: {
        __caption__: ''
      }
    },
    EditSQLGenerator: {
      name: {
        __caption__: 'Название стадии'
      },
      description: {
        __caption__: 'Описание'
      },
      sQLPath: {
        __caption__: ''
      },
      typeMapSQLStr: {
        __caption__: ''
      },
      typeMapSQL: {
        __caption__: 'Карта типов'
      },
      additionalPluginsSettings: {
        __caption__: 'Дополнительные настройки'
      },
      additionalPluginsSettingsStr: {
        __caption__: ''
      },
      moduleSettings: {
        __caption__: '',
        moduleSettingType: {
          __caption__: 'ModuleSettingTyp',
          name: {
            __caption__: 'Название ModuleSettingType'
          }
        },
        valueXML: {
          __caption__: 'ValueXML'
        },
        moduleSettingTypeName: {
          __caption__: ''
        },
        stage: {
          __caption__: 'Stage'
        }
      }
    },
    EditVBGenerator: {
      name: {
        __caption__: 'Название стадии'
      },
      description: {
        __caption__: 'Описание'
      },
      company: {
        __caption__: ''
      },
      product: {
        __caption__: ''
      },
      copyright: {
        __caption__: ''
      },
      version: {
        __caption__: ''
      },
      dataObjectNameSpace: {
        __caption__: ''
      },
      sourceCodeVBPath: {
        __caption__: ''
      },
      typeMapVBStr: {
        __caption__: ''
      },
      typeMapVB: {
        __caption__: ''
      },
      defaultBaseClass: {
        __caption__: ''
      },
      defaultDetailArrayClass: {
        __caption__: ''
      },
      defaultEditScriptName: {
        __caption__: ''
      },
      defaultListScriptName: {
        __caption__: ''
      },
      signAssemblies: {
        __caption__: ''
      }
    },
    FdPreloadMetadata: {
      additionalPluginsSettingsStr: {
        __caption__: ''
      },
      auditEnabled: {
        __caption__: ''
      },
      company: {
        __caption__: ''
      },
      copyright: {
        __caption__: ''
      },
      dataObjectNameSpace: {
        __caption__: ''
      },
      defaultAccessType: {
        __caption__: ''
      },
      defaultBaseClass: {
        __caption__: ''
      },
      defaultDetailArrayClass: {
        __caption__: ''
      },
      defaultEditScriptName: {
        __caption__: ''
      },
      defaultListScriptName: {
        __caption__: ''
      },
      defaultWriteMode: {
        __caption__: ''
      },
      doNotDeleteExtraTables: {
        __caption__: ''
      },
      indexComment: {
        __caption__: ''
      },
      isAuditDatabaseLocal: {
        __caption__: ''
      },
      isReportDatabaseLocal: {
        __caption__: ''
      },
      lastIndexDate: {
        __caption__: ''
      },
      operationsEnumNamespace: {
        __caption__: ''
      },
      operationsEnumPacket: {
        __caption__: ''
      },
      product: {
        __caption__: ''
      },
      realDataObjectNameSpace: {
        __caption__: ''
      },
      scriptNamespace: {
        __caption__: ''
      },
      scriptPacket: {
        __caption__: ''
      },
      serializedIndex: {
        __caption__: ''
      },
      signAssemblies: {
        __caption__: ''
      },
      sourceAzStoragePath: {
        __caption__: ''
      },
      sourceCodeCSPath: {
        __caption__: ''
      },
      sourceCodeVBPath: {
        __caption__: ''
      },
      sourceControlUri: {
        __caption__: ''
      },
      sQLPath: {
        __caption__: ''
      },
      typeMapAccessStr: {
        __caption__: ''
      },
      typeMapCSStr: {
        __caption__: ''
      },
      typeMapOracleStr: {
        __caption__: ''
      },
      typeMapPostgreStr: {
        __caption__: ''
      },
      typeMapSQLStr: {
        __caption__: ''
      },
      typeMapVBStr: {
        __caption__: ''
      },
      useSourceControl: {
        __caption__: ''
      },
      version: {
        __caption__: ''
      },
      createUser: {
        __caption__: ''
      },
      createDate: {
        __caption__: ''
      },
      changeUser: {
        __caption__: ''
      },
      changeDate: {
        __caption__: ''
      },
      name: {
        __caption__: ''
      },
      description: {
        __caption__: ''
      },
      nameStr: {
        __caption__: ''
      },
      configuration: {
        __caption__: '',
        name: {
          __caption__: ''
        }
      }
    },
    FormDesigner_ProjectE: {
      name: {
        __caption__: 'Название'
      },
      description: {
        __caption__: 'Описание'
      },
      product: {
        __caption__: ''
      },
      configuration: {
        __caption__: '',
        project: {
          __caption__: '',
          repository: {
            __caption__: ''
          }
        }
      }
    },
    FormDesigner_ProjectL: {
      name: {
        __caption__: 'Название'
      },
      description: {
        __caption__: 'Описание'
      },
      product: {
        __caption__: ''
      },
      configuration: {
        __caption__: '',
        project: {
          __caption__: '',
          repository: {
            __caption__: ''
          }
        }
      }
    },
    Generations: {
      name: {
        __caption__: ''
      },
      generations: {
        __caption__: '',
        userName: {
          __caption__: 'Пользователь'
        },
        state: {
          __caption__: 'Состояние'
        },
        startTime: {
          __caption__: 'Время старта'
        },
        percentComplete: {
          __caption__: '% выполнения'
        },
        endTime: {
          __caption__: 'Время окончания'
        },
        stage: {
          __caption__: 'Имя стадии',
          name: {
            __caption__: 'Имя стадии'
          }
        },
        generationReason: {
          __caption__: 'Действие'
        }
      }
    },
    Generator: {
      name: {
        __caption__: ''
      },
      description: {
        __caption__: ''
      },
      company: {
        __caption__: ''
      },
      product: {
        __caption__: ''
      },
      copyright: {
        __caption__: ''
      },
      version: {
        __caption__: ''
      },
      dataObjectNameSpace: {
        __caption__: ''
      },
      sourceCodeCSPath: {
        __caption__: ''
      },
      sourceCodeVBPath: {
        __caption__: ''
      },
      sQLPath: {
        __caption__: ''
      },
      typeMapCSStr: {
        __caption__: ''
      },
      typeMapVBStr: {
        __caption__: ''
      },
      typeMapSQLStr: {
        __caption__: ''
      },
      typeMapAccessStr: {
        __caption__: ''
      },
      typeMapOracleStr: {
        __caption__: ''
      },
      typeMapPostgreStr: {
        __caption__: ''
      },
      defaultBaseClass: {
        __caption__: ''
      },
      defaultDetailArrayClass: {
        __caption__: ''
      },
      connectionString: {
        __caption__: ''
      },
      oracleConnectionString: {
        __caption__: ''
      },
      postgreConnectionString: {
        __caption__: ''
      },
      additionalPluginsSettingsStr: {
        __caption__: ''
      },
      defaultEditScriptName: {
        __caption__: ''
      },
      defaultListScriptName: {
        __caption__: ''
      },
      sourceAzStoragePath: {
        __caption__: ''
      },
      operationsEnumPacket: {
        __caption__: ''
      },
      operationsEnumNamespace: {
        __caption__: ''
      },
      signAssemblies: {
        __caption__: ''
      },
      auditEnabled: {
        __caption__: ''
      },
      isAuditDatabaseLocal: {
        __caption__: ''
      },
      defaultWriteMode: {
        __caption__: ''
      },
      isReportDatabaseLocal: {
        __caption__: ''
      },
      typeDefinitions: {
        __caption__: '',
        name: {
          __caption__: ''
        },
        caption: {
          __caption__: ''
        },
        mapTypeName: {
          __caption__: ''
        },
        mapTypeAssemblyName: {
          __caption__: ''
        },
        formatAttribute: {
          __caption__: ''
        }
      },
      controlTypes: {
        __caption__: '',
        name: {
          __caption__: 'Имя'
        },
        designerHtmlTemplate: {
          __caption__: 'Шаблон'
        },
        designerMetadataXml: {
          __caption__: 'Метаданные дизайна'
        },
        editedType: {
          __caption__: '',
          name: {
            __caption__: ''
          }
        }
      },
      moduleSettings: {
        __caption__: '',
        moduleSettingType: {
          __caption__: 'ModuleSettingTyp',
          name: {
            __caption__: 'Название ModuleSettingType'
          }
        },
        valueXML: {
          __caption__: 'ValueXML'
        },
        moduleSettingTypeName: {
          __caption__: ''
        },
        stage: {
          __caption__: 'Stage'
        }
      }
    },
    IntelliSearchPluginIndexing: {
      name: {
        __caption__: ''
      },
      changeDate: {
        __caption__: ''
      },
      lastIndexDate: {
        __caption__: ''
      },
      indexComment: {
        __caption__: ''
      },
      typeMapCSStr: {
        __caption__: ''
      },
      serializedIndex: {
        __caption__: ''
      }
    },
    IntelliSearchPluginLight: {
      name: {
        __caption__: ''
      },
      changeDate: {
        __caption__: ''
      },
      lastIndexDate: {
        __caption__: ''
      },
      indexComment: {
        __caption__: ''
      },
      configuration: {
        __caption__: '',
        name: {
          __caption__: ''
        },
        project: {
          __caption__: '',
          name: {
            __caption__: ''
          },
          repository: {
            __caption__: '',
            name: {
              __caption__: ''
            }
          }
        }
      }
    },
    IntelliSearchPluginSearch: {
      name: {
        __caption__: ''
      },
      changeDate: {
        __caption__: ''
      },
      lastIndexDate: {
        __caption__: ''
      },
      indexComment: {
        __caption__: ''
      },
      configuration: {
        __caption__: '',
        name: {
          __caption__: ''
        },
        project: {
          __caption__: '',
          name: {
            __caption__: ''
          },
          repository: {
            __caption__: '',
            name: {
              __caption__: ''
            }
          }
        }
      },
      serializedIndex: {
        __caption__: ''
      }
    },
    LightStage: {
      company: {
        __caption__: ''
      },
      product: {
        __caption__: ''
      },
      sourceCodeCSPath: {
        __caption__: ''
      },
      moduleSettings: {
        __caption__: '',
        moduleSettingType: {
          __caption__: 'ModuleSettingTyp',
          name: {
            __caption__: 'Название ModuleSettingType'
          }
        },
        valueXML: {
          __caption__: 'ValueXML'
        },
        moduleSettingTypeName: {
          __caption__: ''
        },
        stage: {
          __caption__: 'Stage'
        }
      }
    },
    ListDataObjectTypes: {
    },
    ListFormView: {
      name: {
        __caption__: 'Название'
      },
      description: {
        __caption__: 'Описание'
      },
      changeUser: {
        __caption__: 'Редактор'
      },
      changeDate: {
        __caption__: 'Дата изменения'
      },
      createUser: {
        __caption__: 'Создатель'
      },
      createDate: {
        __caption__: 'Дата создания'
      },
      configuration: {
        __caption__: ''
      }
    },
    Prototyping: {
    },
    SearchRepObjView: {
      name: {
        __caption__: ''
      }
    },
    ViewPeeker: {
    }
  }
};

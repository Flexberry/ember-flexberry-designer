...
function findDiagrams(store, currentProjectContext, firstClassName, secondClassName, link) {
  ...
  let diagramsWithFirstClass = Ember.A(diagramsCurrentStage).filter(function(diagram) {
    let caseObjectsString = diagram.get('caseObjectsString');
    return !Ember.isNone(caseObjectsString) && caseObjectsString.indexOf(`Class:(${firstClassName})`) !== -1;
  });

  if (!Ember.isNone(secondClassName)) {
    let diagramsWithClasses = Ember.A(diagramsWithFirstClass).filter(function(diagram) {
      let caseObjectsString = diagram.get('caseObjectsString');
      return !Ember.isNone(caseObjectsString) && caseObjectsString.indexOf(`Class:(${secondClassName})`) !== -1;
    });

    let linkId = link.get('id');
    diagrams = Ember.A(diagramsWithClasses).filter(function(diagram) {
      ...
      return !Ember.isNone(repositoryObject);
    });
  } else {
...
let updateInheritanceOnDiagram = function(store, devInheritance) {
  ...
  if (!Ember.isNone(parentDiagrams.findBy('name', `AutogeneratedDiagramForClass_${parentClassName}`)) &&
  Ember.isNone(parentDiagrams.findBy('name', `AutogeneratedDiagramForClass_${childClassName}`))) {
